...
DHTMLgoodies_dragDrop.prototype = {
    ...
    init: function ()
    {
        if (!standardObjectsCreated)
            DHTMLgoodies_createStandardObjects();	// This line starts all the init methods
    ...
    addSource: function (sourceId, slideBackAfterDrop, xAxis, yAxis, dragOnlyWithinElId, functionToCallOnDrag)
    {
        if (!functionToCallOnDrag)
            functionToCallOnDrag = false;
        if (!this.dragDropSourcesArray)
            this.dragDropSourcesArray = new Array();
        if (!document.getElementById(sourceId))
            alert('The source element with id ' + sourceId + ' does not exists');
    ...
    addTarget: function (targetId, functionToCallOnDrop)
    {
        ...
        if (!document.getElementById(targetId))
            alert('The target element with id ' + targetId + ' does not exists');
   ...
   ,
    ...
    __moveDragableElement: function (e)
    {
        ...
        if (referenceToDragDropObject.currentEl_allowX) {
            ...
            if (referenceToDragDropObject.drag_maxX) {
                ...
                if (leftPos > tmpMaxX)
                    leftPos = tmpMaxX
                if (leftPos < referenceToDragDropObject.drag_minX)
                    leftPos = referenceToDragDropObject.drag_minX;
            }
        ...
        if (referenceToDragDropObject.currentEl_allowY) {
            ...
            if (referenceToDragDropObject.drag_maxY) {
                ...
                if (topPos > tmpMaxY)
                    topPos = tmpMaxY;
                if (topPos < referenceToDragDropObject.drag_minY)
                    topPos = referenceToDragDropObject.drag_minY;
    ...
    __processSlide: function (numId, currentX, currentY, targetX, targetY)
    {
        ...
        if (slideY < 3 && Math.abs(slideX) < 10)
            slideY = 3;	// 3 is minimum slide value
        if (slideX < 3 && Math.abs(slideY) < 10)
            slideX = 3;	// 3 is minimum slide value


        if (currentX > targetX)
            slideX *= -1;	// If current x is larger than target x, make slide value negative<br>
        if (currentY > targetY)
            slideY *= -1;	// If current y is larger than target x, make slide value negative
...
function dropItems(idOfDraggedItem, targetId, x, y)
{
    ...
    if (subDivs.length > 0 && targetId != 'capitals')
